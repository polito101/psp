...
- La idempotencia de creación de intents en `POST /v2/payments` usa `hashCreatePaymentIntentPayload` en `apps/psp-api/src/payments-v2/create-payment-intent-payload-hash.ts` (SHA-256 sobre un objeto canónico del DTO usado por `executeProviderOperation('create')`).
