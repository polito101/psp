...
| `merchant-onboarding` | Si | No | No | Parcial | Unit `merchant-onboarding.service.spec.ts` cubre creación pública neutral con merchant inactivo, checklist, token y eventos post-email sent/failed; link público solo en development/sandbox; submit por token válido; rechazo de tokens expirados/usados/revocados; approve con `APPROVED` → activación `ACTIVE` y evento `merchant_activated`; reject con checklist bloqueado; reenvío de link. Task 4 agregará controllers HTTP e integración. |
