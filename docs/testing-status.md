...
| `backoffice BFF` | Si (proxy + guards + login RL + portal mode) | No | Playwright smoke | Parcial | Vitest: proxy (`proxy.spec.ts`: merchant vs admin login path, sesión cruzada ignorada), mutación interna (`internal-mutation-guard.spec.ts`), rate limit login (`login-rate-limit.spec.ts`, barrido/evicción), resolución IP (`client-ip.spec.ts`), `enforceInternalRouteAuth` con `BACKOFFICE_PORTAL_MODE` (`internal-route-auth.spec.ts`), rutas API (`provider-health`, `payments`, `auth/session`: modos segregados por portal + RL). E2E: `e2e/auth-and-rbac.spec.ts` usa portal **admin** (redirect `/admin/login` sin cookie; login API admin + `/merchants`). |
