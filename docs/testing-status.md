...
| `merchant-onboarding` | Si | No | No | Parcial | Unit `merchant-onboarding.service.spec.ts` cubre creación pública neutral con merchant inactivo, checklist, token y eventos; submit por token válido; rechazo de tokens expirados/usados; approve/reject; reenvío de link. Task 4 agregará controllers HTTP e integración. |
