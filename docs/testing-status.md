...
| `internal endpoints` | Si (guards) | Si | Si | Cubierto | Mismo surface; guard `InternalSecretGuard` valida `X-Internal-Secret` y, si el BFF envía `X-Backoffice-Role: merchant`, exige `X-Backoffice-Merchant-Id` alineado con path/query (`merchantId` obligatorio en listados agregados; sin acceso a `ops/metrics`; finance path debe coincidir). Unit ampliado en `internal-secret.guard.spec.ts`. Detalle pago: `getOpsPaymentDetail` rechaza cross-merchant con `backofficeMerchantScopeId`. Backoffice: Vitest en `internal-route-auth`, `session-claims`, `internal-route-scope`, rutas internas, `middleware`, `POST /api/auth/session`. |
