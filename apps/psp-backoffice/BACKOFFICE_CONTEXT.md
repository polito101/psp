...
- Mutaciones BFF (`POST`/`PATCH` bajo `/api/internal/*`): el navegador debe enviar cabecera `X-Backoffice-Mutation: 1`; si existe cabecera `Origin`, debe coincidir con `request.nextUrl.origin` **o** con el origen público reconstruido desde `Host` + `X-Forwarded-Proto` (primer segmento si la lista es coma-separada; opcional `X-Forwarded-Host`) para que mutaciones no fallen en proxy TLS (p. ej. Render + Cloudflare cuando `nextUrl` es interno `http://127.0.0.1:…`). La sesión/RBAC sigue aplicándose después.
