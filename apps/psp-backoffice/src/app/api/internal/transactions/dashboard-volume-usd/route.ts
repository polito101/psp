...
import { z } from "zod";
import type { OpsDashboardVolumeUsdResponse } from "@/lib/api/contracts";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await proxyInternalGet<OpsDashboardVolumeUsdResponse>({
      path: "/api/v2/payments/ops/dashboard/volume-usd",
      searchParams: params,
