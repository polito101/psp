...
import { z } from "zod";
import type { OpsPaymentsSummaryHourlyResponse } from "@/lib/api/contracts";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await proxyInternalGet<OpsPaymentsSummaryHourlyResponse>({
      path: "/api/v2/payments/ops/transactions/summary-hourly",
      searchParams: params,
