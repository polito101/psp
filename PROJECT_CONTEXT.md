...
- Optimizacion DB (webhooks inbound): `Payment` indexa `(@@index([selectedProvider, providerRef]))` para resolver lookup por `(selectedProvider, providerRef)` sin scans bajo volumen.
