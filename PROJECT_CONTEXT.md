...
- Concurrencia en operaciones v2: `PaymentOperation` (lock por `paymentId+operation`) evita ejecutar dos veces `capture/refund/cancel` bajo carrera; la fila se crea/actualiza a `processing` y se marca `done` al completar. Si la operacion queda atascada, se considera stale tras `PAYMENTS_V2_OPERATION_LOCK_STALE_MS` (default 30s) y permite takeover. Tras `claim` con `proceed`, `capture`/`cancel`/`refund` llaman `completePaymentOperation` en `finally` (incl. early return y excepciones) para no dejar locks en `processing`.
