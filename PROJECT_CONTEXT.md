...
- Create intent v2: el comercio **no** envía proveedor en el body; el primer intento sigue `PAYMENTS_PROVIDER_ORDER` (y fallbacks/circuitos). El contrato público preferido usa **importe decimal** (`amount`), `channel`, `language`, `orderId`, `description`, `notificationUrl`, `returnUrl`, `cancelUrl` y `customer` ( país ISO2, email, nombre); se mantiene compatibilidad con `amountMinor` + `currency` para clientes legacy. La huella de idempotencia discrimina `v:1` vs `v:2`.
