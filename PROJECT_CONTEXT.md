...
- **Adaptador HTTP genérico (evolutivo):** las rutas de pago referencian plantillas de petición; las llamadas salientes al PSP se construyen y normalizan en código de aplicación (payload enmascarado en `ProviderLog`). Entrada de callbacks del proveedor y reenvío controlado al `notificationUrl` del comercio se documentan en `apps/psp-api/README.md`; operación y revisión en **backoffice** (`/payment-providers`, `/payment-methods`, detalle de pago con pestañas de logs y notificaciones).
