...
- Merchant admin account: `Merchant` almacena campos de cuenta administrativa (`email`, nombre y teléfono de contacto, URL web, `mid`, estado de registro/número, `industry`). El backoffice admin en `/merchants/:merchantId/admin` opera sobre esos campos; multi-shop / `Shop` sigue fuera de alcance.
...
- Create intent v2: el comercio **no** envía proveedor en el body; el primer intento sigue `PAYMENTS_PROVIDER_ORDER` (y fallbacks/circuitos). El contrato público del `POST` usa **importe decimal** (`amount`), `currency` ISO 4217, `channel`, `language`, `orderId`, `description`, `notificationUrl`, `returnUrl`, `cancelUrl` y `customer` (país ISO2, email, nombre). El `ValidationPipe` global (`whitelist` + `forbidNonWhitelisted`) rechaza cuerpos con propiedades no declaradas en el DTO; **no** hay forma soportada de crear el intent usando solo `amountMinor` en el JSON público. **Conversión decimal → minor:** reglas codificadas en `apps/psp-api/src/payments-v2/decimal-amount-to-minor.ts`: la mayoría de divisas usa dos decimales (p. ej. EUR/USD → factor ×100), un conjunto sin fracción (p. ej. JPY, KRW → importe mayor como unidad mínima) y otro con tres decimales (p. ej. KWD, BHD → ×1000); el resto cae en dos decimales. Tras `Math.round`, el minor debe ser entero seguro **≥ 1** y ≤ **2_147_483_647** (tope Prisma INTEGER en `Payment.amountMinor`); si no → `400` (DTO y servicio alineados). Idempotencia de creación: huella SHA del payload canónico (`hashCreatePaymentIntentPayload`, esquema interno `v: 2` en la serialización estable); filas antiguas sin `create_payload_hash` se validan en replay por equivalencia de importe en minor derivado del decimal, divisa y `paymentLinkId`.
- **Enrutado dinámico (configuración):** modelos `PaymentProviderConfig`, `PaymentMethodRoute`, `PaymentMethodRouteCurrency` y `MerchantProviderRate` (migración `20260506120000_dynamic_payment_routing_config`) describen integración HTTP por proveedor, plantillas `REDIRECT_SIMPLE` / `SPEI_BANK_TRANSFER`, pesos y límites por divisa; el runtime de create puede evolucionar a selección ponderada leyendo estas tablas, mientras el orquestador **mock** sigue decidiendo por `PAYMENTS_PROVIDER_ORDER`. Estado de negocio expuesto hacia comercio y callbacks posteriores se alinea con el enum **`DynamicPaymentStatus`** (`PENDING`, `PAID`, `FAILED`, `EXPIRED`) donde aplica (p. ej. entregas en `PaymentNotificationDelivery`).
- **API interna de esa configuración (backoffice ops):** `GET/POST /api/v2/payments/ops/configuration/providers`, `PATCH …/providers/:providerId`, `GET/POST …/configuration/routes`, `PATCH …/routes/:routeId` y `PATCH …/routes/:routeId/weight`, `GET/POST …/configuration/merchants/:merchantId/provider-rates`. `InternalSecretGuard` trata `/payments/ops/configuration` como **solo admin** (fail-closed: rol `merchant` no autorizado). Implementación en `PaymentsV2OpsConfigurationController` + `PaymentsV2OpsConfigurationService` (Prisma).
- **Adaptador HTTP genérico / `ProviderLog` (evolutivo):** modelo y DDL listos (`ProviderLog`) para registrar peticiones/responses externas cuando el runtime las persista; hasta completar ese cableado algunos despliegues pueden ver la tabla vacía en detalle ops. Contrato orientado a payload enmascarado en vistas operativas.
- **`notificationUrl` (create v2) vs webhooks de merchant (`webhookUrl`):** la URL de notificación declarada por el comercio se persiste (`Payment.notification_url`). El **`POST ops …/notifications/…/resend`** ejecuta un **único** `POST` server-side con carga enmascarada: políticas anti-SSRF en **`merchant-notification-url.policy`** (HTTPS en producción, sin credenciales en URL, bloqueo de IPs/metadata/rangos no públicos en literal **y** vía **DNS** previo al `fetch`), lectura del body acotada (64KiB) antes de persistir metadatos, `fetch` con **`redirect: 'manual'`**, y endpoint **`InternalSecretGuard` admin-only** (el portal merchant no debe poder dispararlo). Los cambios financieros **capture/refund** continúan notificándose mediante la cola **`webhook_deliveries`**/`WebhooksService` hacia **`Merchant.webhookUrl`**, que es el canal estable hoy día. Auditoría relacionada aparece como `PaymentNotificationDelivery` donde el flujo la cree (p. ej. reintentos desde backoffice).
...
- **Correlación HTTP (Payments v2 y rutas del mismo módulo):** middleware `CorrelationIdMiddleware` (`AsyncLocalStorage` en `apps/psp-api/src/common/correlation/`) aplicado a `PaymentsV2Controller` (API comercio `/api/v2/payments`), `PaymentsV2InternalController` y `PaymentsV2OpsConfigurationController` (ops interno bajo el mismo prefijo). Entrada: cabeceras opcionales **`X-Request-Id`** (prioritaria) y **`X-Correlation-Id`** (solo si falta la primera); valores inválidos o ausentes → **UUID v4** generado por petición. Salida: siempre **`X-Request-Id`** en la respuesta (el valor aceptado o el generado). El id se propaga a `PaymentsV2Service` vía `CorrelationContextService`, se añade como `correlationId` en logs JSON `payments_v2.*` relevantes, en `payments_v2.provider_attempt` y en `http.request` cuando el contexto existe. Alcance: **misma petición HTTP y mismo proceso Node** (reintentos de adapter y varios `PaymentAttempt` comparten el id); propagación a workers u otros servicios queda fuera de esta fase.
