...
- Endpoint interno `GET /api/v2/payments/ops/payments/:paymentId`: detalle operativo de un pago por id interno con hasta **200** `PaymentAttempt` más recientes (orden cronológico ascendente en payload), más `attemptsTotal` y `attemptsTruncated` cuando el historial supera ese tope. Metadatos: `idempotencyKey`, `paymentLinkId`, `rail`, timestamps. Consumido por el backoffice vía BFF `GET /api/internal/payments/:paymentId`.
...
- Optimizacion DB (webhooks inbound): `Payment` indexa `(@@index([selectedProvider, providerRef]))` para resolver lookup por `(selectedProvider, providerRef)` sin scans bajo volumen. En deploy, este índice se crea con `CREATE INDEX CONCURRENTLY` como paso operacional (`npm -w apps/psp-api run prisma:ops:indexes`) porque `prisma migrate deploy` corre migraciones en transacción y `CONCURRENTLY` no es compatible dentro de transacciones.
...
- Se amplió la batería de tests de Stripe webhook:
  - Integration inbound: matriz completa (`payment_intent.succeeded|payment_failed|canceled`, `charge.refunded`, casos `payment_not_found`, `missing_provider_ref`, `unsupported_event_type`) y hardening de firma/JSON/payload.
