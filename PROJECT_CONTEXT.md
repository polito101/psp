...
- Endpoint interno `GET /api/v2/payments/ops/transactions/volume-hourly`: serie horaria UTC (24 buckets) de volumen acumulado (`amount_minor`) de pagos `succeeded` para **hoy vs ayer**; filtros opcionales `merchantId`, `provider`, `currency` (default EUR). Expuesto al panel vía BFF `GET /api/internal/transactions/volume-hourly`.
